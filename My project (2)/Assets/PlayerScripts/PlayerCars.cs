...
public class PlayerCars : MonoBehaviour
{
    ...
    private CircleCheck сircleCheck;
    ...
    private void Start()
    {
       ...
       ghostTrail = FindObjectOfType<GhostTrail>();
        ...
        сircleCheck = FindObjectOfType<CircleCheck>();
        if (ghostTrail == null)
        {
        ...
        if (File.Exists(filePath))
        {
            ...
            currentPlayerIndex = playerData.currentPlayerIndex; // Установим текущего игрока
    ...
    {
        // Сначала деактивируем машину текущего игрока
        DeactivateCurrentPlayerCar(currentPlayerIndex);

        // Увеличиваем индекс текущего игрока на 1
        currentPlayerIndex++;

        // Если текущий индекс больше максимального индекса игрока, игра завершена
        ...
        // Активируем машину следующего игрока
        ActivateCurrentPlayerCar(currentPlayerIndex, selectedCarsIndices);

        // Показываем следы привидений для выбранных машин
        ShowAllGhostTrails(selectedCarsIndices);

        // Вызываем метод SetPCControl с аргументом true только если игрок выбрал управление клавиатурой
        ...
        // Показываем Fullscreen только если currentPlayerIndex <= maxPlayerIndex
        if (currentPlayerIndex <= maxPlayerIndex)
        {
